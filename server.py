...
def predict():
    ...
    ipo_datetime_str = ipo_date.strftime("%Y-%m-%d %H:%M:%S")
    ipo_datetime = datetime.fromisoformat(ipo_datetime_str)
    print(f"IPO Date: {ipo_datetime}")
    #current_datetime = datetime.fromisoformat(dt.datetime.now())
    if dt.datetime.now() - ipo_datetime < dt.timedelta(days=105):
        return render_template('error.html', error=f"{ticker} has not been publicly traded long enough to make valuable predictions")
