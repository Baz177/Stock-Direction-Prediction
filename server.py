...
def fetch_and_preprocess_data(ticker):
    ...
    change = df['Close_Change'].iloc[-1]  # Last close change
    ...
    return pred_prob, close_price, change, None
...
def predict():
    ...
    pred_prob, close_price, change, error = fetch_and_preprocess_data(ticker)
    ...
    tomorrow_date = dt.datetime.now() + pd.Timedelta(days=1)
